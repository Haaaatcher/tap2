...
# conda activate tapp
pip install ./tapp-0.06-py3-none-any.whl
```

启动 `tapp` 的用户界面十分简单，只需在终端中输入 `tapu`（含义是：`TAPP's UI`）命令，输出类似如下信息则表示启动成功，在浏览器中进入[输出的连接](http://127.0.0.1:7860)就可以使用 `tapp` 了。
...
如果需要供外部访问，可以使用 `--host` 和 `--port` 参数分别指定 IPv4 地址和端口号（默认为`7860`），例如：
...
`tapp` 的 UI 基于 [Gradio](https://www.gradio.app/) 框架构建，包括`物理性能`、`力学性能`、`批量模式`和`模板下载`四个模块，在网页版用户界面中通过选项卡进行模块切换。

![利用选项卡进行模块切换](./assets/tab.png)

- **物理性能**

  - 在物理性能界面的左侧输入钛合金的元素组成和工艺参数，点击提交按钮，界面右侧将显示由 `TAPP` 模型计算的物理性能。
...
- **力学性能**
  ...
  - 在力学性能界面的左侧输入钛合金的元素组成和工艺参数，点击提交按钮，界面右侧将显示由 `TAPP` 模型计算的力学性能。
...
`tapp` 支持使用 `Python API` 和 `OpenAPI` 进行钛合金性能预测，均支持**单点预测**和**批量预测**两种方式。

#### 2.3.1 Python API

在 Python 脚本中直接调用相关类：`TAPPInput` 和 `TAPPModelInfer`，下面是示例代码：

- **单点预测**

```python
from tapp import TAPPInput, TAPPModelInfer

# 创建TAPP推理器实例
tapp_model_infer = TAPPModelInfer()

# 用字典定义单条输入数据
data = {"Prop": "D", "Ti": 90.0, "H": 0.0, "B": 0.0, "C": 0.0, "N": 0.0, "O": 0.0, "Al": 6.0, "Si": 0.0, "Cr": 0.0, "Fe": 0.0, "Ni": 0.0, "Cu": 0.0, "Zr": 0.0, "Nb": 0.0, "Mo": 0.0, "V": 4.0, "Sn": 0.0, "HTT": 800.0}

# 创建TAPP输入实例
data = TAPPInput(**data)

# 调用TAPP进行推理
result = tapp_model_infer(data)
...
# 用字典列表定义多条输入数据
data = [
    {"Prop": "D", "Ti": 90.0, "H": 0.0, "B": 0.0, "C": 0.0, "N": 0.0, "O": 0.0, "Al": 6.0, "Si": 0.0, "Cr": 0.0, "Fe": 0.0, "Ni": 0.0, "Cu": 0.0, "Zr": 0.0, "Nb": 0.0, "Mo": 0.0, "V": 4.0, "Sn": 0.0, "HTT": 800.0},
    {"Prop": "D", "Ti": 97.0, "H": 0.0, "B": 0.0, "C": 0.0, "N": 0.0, "O": 0.0, "Al": 3.0, "Si": 0.0, "Cr": 0.0, "Fe": 0.0, "Ni": 0.0, "Cu": 0.0, "Zr": 0.0, "Nb": 0.0, "Mo": 0.0, "V": 0.0, "Sn": 0.0, "HTT": 800.0}
]

# 创建TAPP输入实例
data = [TAPPInput(**_) for _ in data]

# 调用TAPP进行推理
results = tapp_model_infer(data)
...
- **注意事项**
  ...
  - 示例中输入数据的每一个字段都不可缺少，且元素组成的总和必须为 `100`。

  - 当预测力学性能（`屈服强度`、`抗拉强度`、`硬度`、`霍尔佩奇系数`）时，还需要提供晶粒尺寸字段（`GS`），单位为微米。

  - 当进行批量计算时，需确保每一个样本的性能字段（`Prop`）是一致的。
...
使用 `taps` 命令（含义是 `TAPP's Server`）开启服务模式，然后通过 HTTP 请求来调用 `tapp` 的功能。服务模式基于 [FastAPI](https://fastapi.tiangolo.com/zh/) 框架构建，用户可以通过其 [Docs 页面](http://127.0.0.1:8000/docs) 查看接口规范。
...
如果需要供外部访问，可以使用 `--host` 和 `--port` 参数指定 IPv4 地址和端口号（默认为`8000`），例如：
...
`tapp` 的性能预测使用了基于 `MoE` 的机器学习算法：建立若干个**元素专家**和一个**专家评分器**，元素专家接收 Ti 元素和对应元素的浓度作为输入，并输出预测的性能值，专家评分器则接收全部元素组成作为输入，输出对各个元素专家的评分，最后将各个元素专家给出的性能值和评分器给出的专家评分进行加权求和，得到最终的性能值。
