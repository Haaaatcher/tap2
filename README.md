...
# conda activate tapp
pip install ./tap2-0.07-py3-none-any.whl
```

在本地启动 `tap2` 的用户界面十分简单，只需在终端中启动 `tapu`（含义是：`TAP2's UI`）命令，输出如下信息则表示启动成功，在浏览器中进入[输出的连接](http://127.0.0.1:7860)就可以使用 `tap2` 了。
...
Loading models: 100%|████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████| 15/15 [00:00<00:00, 21.27it/s]
* Running on local URL:  http://127.0.0.1:7860
* To create a public link, set `share=True` in `launch()`.
```

如果需要提供给外部访问，可以使用 `tapu` 命令的 `--host` 和 `--port` 参数分别指定 IPv4 地址和端口号（默认为`7860`），例如：
...
`tap2` 的 UI 基于 [Gradio](https://www.gradio.app/) 框架构建，包括`物理性能`、`力学性能`、`相比例`、`β-转变温度`（**即将上线**）、`批量模式` 和 `模板下载` 6 个模块，在用户界面中通过选项卡进行模块切换。
...
- **力学性能**
  ...
  - 工艺参数支持：
    - 快捷模式：`热处理温度`、`晶粒尺寸`。
    - 高级模式：热变形（`温度`、`真实应变`、`应变速率`、`初始晶粒尺寸`、`β相变温度`）。
...
- **相比例**
  ...
  - 在相比例界面的左侧输入钛合金的元素组成和工艺参数，点击提交按钮，界面右侧将显示由 `TAPP` 模型计算的相比例和相应的饼图。
  - 点击重置按钮可以清空全部输入和输出，并将其恢复至默认值。
...
如果需要提供给外部访问，可以使用 `--host` 和 `--port` 参数指定 IPv4 地址和端口号（默认为`8000`），例如：
